...
function start(callback) {
    var mqttCfg = config.getConfig().mqtt;
    var options = {
        keepalive: 0,
        connectTimeout: 60 * 60 * 1000,
        ca: mqttCfg.ca ? mqttCfg.ca : null,
        cert: mqttCfg.cert ? mqttCfg.cert : null,
        key: mqttCfg.key ? mqttCfg.key : null
    };

    config.getLogger().info(context, 'Starting MQTT binding');

    if (mqttCfg && mqttCfg.username && mqttCfg.password) {
        options.username = config.getConfig().mqtt.username;
        options.password = config.getConfig().mqtt.password;
    }

    mqttClient = mqtt.connect(
        (mqttCfg.protocol ? mqttCfg.protocol : 'mqtt') + '://' + mqttCfg.host + ':' + mqttCfg.port,
        options
    );
